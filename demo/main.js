...
function meetingstatuscb(status, result) {
  ...
  switch (status) {
    case ZoomMeetingStatus.MEETING_STATUS_CONNECTING:
      YUVWindow
        ? YUVWindow.webContents.send('main-process-meetingstatus', 'connecting')
        : null;
      break;
    ...
    case ZoomMeetingStatus.MEETING_STATUS_INMEETING:
      // showInMeetingWindow();
      YUVWindow
        ? YUVWindow.webContents.send('main-process-meetingstatus', 'inmeeting')
        : null;
      break;
...
function OnDirectShareStatusUpdate(status) {
  ...
  startjoinWindow
    ? startjoinWindow.webContents.send(
        'main-process-onDirectShareStatusUpdate',
        status
      )
    : null;
}
...
function onVideoRawDataReceived(databuf, format, receivers) {
  ...
  YUVWindow
    ? YUVWindow.webContents.send('main-process-videomessages', videoObj)
    : null;
}
...
function onShareRawDataReceived(databuf, format, receivers) {
  ...
  YUVWindow
    ? YUVWindow.webContents.send('main-process-sharemessages', shareObj)
    : null;
}
...
function onMixedAudioRawDataReceived(databuf, data_format) {
  if (YUVWindow) {
    ...
    YUVWindow
      ? YUVWindow.webContents.send('main-process-mixaudiomessages', mixAudioObj)
      : null;
  }
}
...
function onOneWayAudioRawDataReceived(databuf, data_format, nodeid) {
  if (YUVWindow) {
    ...
    YUVWindow
      ? YUVWindow.webContents.send(
          'main-process-onewayaudiomessages',
          oneWayAudioObj
        )
      : null;
  }
...
function customizedresource() {
  ...
  if (zoomcustomizedresource) {
    let retCustomize = zoomcustomizedresource.Resource_AddCustomizedStringResource(
      optCustomizedResouce
    );
    console.log('retCustomize', retCustomize);
    let retURLCustomize = zoomcustomizedresource.Resource_AddCustomizedURLResource(
      optCustomizedURLResouce
    );
    console.log('retURLCustomize', retURLCustomize);
    let retPictureCustomize = zoomcustomizedresource.Resource_AddCustomizedPictureResource(
      optCustomizedPictureResouce
    );
...
let functionObj = {
  ...
  changeFloatoActiveSpkVideoSize: function () {
    console.log(
      'ZoomMeetingUIFloatVideoType',
      ZoomMeetingUIFloatVideoType.FLOATVIDEO_Large
    );
    let obj = {
  ...
  enableAutoFullScreenVideoWhenViewShare: function (bEnable) {
    ...
    let ret = zoomsetgeneral.Setting_EnableAutoFullScreenVideoWhenViewShare(
      opts
    );
  ...
  isAutoFullScreenVideoWhenViewShareEnabled: function (bEnable) {
    ...
    let ret = zoomsetgeneral.Setting_IsAutoFullScreenVideoWhenViewShareEnabled(
      opts
    );
  ...
  isCurrentOSSupportAccelerateGPUWhenShare: function (bEnable) {
    ...
    let ret = zoomsetgeneral.Setting_IsCurrentOSSupportAccelerateGPUWhenShare(
      opts
    );
  ...
  isRemoteControlAllApplicationsEnabled: function (bEnable) {
    ...
    let ret = zoomsetgeneral.Setting_IsRemoteControlAllApplicationsEnabled(
      opts
    );
  ...
  setAudioRawDataCB: function () {
    let SetMixedAudioRawDataCB = zoomaudiorawdata.SetMixedAudioRawDataCB(
      onMixedAudioRawDataReceived
    );
    console.log('SetMixedAudioRawDataCB', SetMixedAudioRawDataCB);
    let SetOneWayAudioRawDataCB = zoomaudiorawdata.SetOneWayAudioRawDataCB(
...
function getJWT() {
  ...
  const ZOOM_SDK_KEY = 'C6Psll8UXJrBuARBNWEWURfmRR3wP3RGhqkY';
  const ZOOM_SDK_SECRET = '809LSR3uSOiLAfxbFLOpFUDPgHJqk0K9FhoP';
